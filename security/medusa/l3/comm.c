...
medusa_answer_t med_decide(struct medusa_evtype_s * evtype, void * event, void * o1, void * o2)
{
	...
	if (!authserver) {
		if (((struct medusa_kclass_s*)evtype->arg_kclass[0].ptr)->unmonitor)
			((struct medusa_kclass_s*)evtype->arg_kclass[0].ptr)->unmonitor((struct medusa_kobject_s *) o1);
		if (((struct medusa_kclass_s*)evtype->arg_kclass[1].ptr)->unmonitor)
			((struct medusa_kclass_s*)evtype->arg_kclass[1].ptr)->unmonitor((struct medusa_kobject_s *) o2);
		MED_UNLOCK_W(registry_lock);
		return MED_OK;
	}
	MED_UNLOCK_W(registry_lock);

	((struct medusa_event_s *)event)->evtype_id = (MCPptr_t)(void*)evtype;
