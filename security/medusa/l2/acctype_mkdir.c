...
medusa_answer_t medusa_mkdir(struct dentry *dentry, int mode)
{
	...
	if (MEDUSA_MONITORED_ACCESS_O(mkdir_access, &inode_security(ndparent.path.dentry->d_inode)))
		retval = medusa_do_mkdir(ndparent.path.dentry, ndupper.path.dentry, mode);
	else
		retval = MED_OK;
	medusa_put_upper_and_parent(&ndupper, &ndparent);
	return retval;
