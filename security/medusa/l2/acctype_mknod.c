...
medusa_answer_t medusa_mknod(struct dentry *dentry, dev_t dev, int mode)
{
	...
	if (MEDUSA_MONITORED_ACCESS_O(mknod_access, &inode_security(ndparent.path.dentry->d_inode)))
		retval = medusa_do_mknod(ndparent.path.dentry, ndupper.path.dentry, dev, mode);
	else
		retval = MED_OK;
	medusa_put_upper_and_parent(&ndupper, &ndparent);
	return retval;
